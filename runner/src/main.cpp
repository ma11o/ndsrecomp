...
#endif
#ifdef NDS_HAVE_MODS
// One call per NDS_MOD_DIR file's nds_mod_init_<stem>() (nds_mod.h),
// generated by runner/CMakeLists.txt. Absent when NDS_MOD_DIR is unset.
extern "C" void nds_mod_run_all_init(void);
#endif
...
int main(int argc, char** argv) {
    ...
    auto boot = [&]() {
        ...
        // Built-in bank hook tables (docs/mod-hooks.md), unconditional --
        // these banks always exist, unlike a title's. A no-op call when no
        // built-in bank was ever regenerated with --hook-seams.
        nds_mod_register_builtin_hook_tables();
#ifndef NDS_BOOTSTRAP_FIRMWARE
        ...
        nds_register_configured_title_hook_tables(rom_sha1.c_str());
        live_overlay_register_cached_banks();
#ifdef NDS_HAVE_MODS
        // Mods arm their hooks by (bank, addr) (nds_hook_enter/leave), so
        // every bank's hook table above must already be registered. Runs
        // once, on this (the emulation) thread, before any guest code.
        nds_mod_run_all_init();
#endif
#ifdef NDS_HAVE_SM64DS_BANKS
        if (sm64ds_title) {
